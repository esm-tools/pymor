...
Gemessen am fertigen Lauf, 534 Dateien:

\begin{center}
\begin{tabular}{lrr}
\toprule
& \textbf{cf + wcrp\_cmip7} & \textbf{aicc} \\
\midrule
HIGH   &   4 & 31 \\
MEDIUM & 304 &  0 \\
\bottomrule
...
Zum Vergleich: vor dieser Runde lagen wir bei 284 MEDIUM auf denselben zwei
Suiten, aber ohne dass aicc ueberhaupt mitlief. Die 35 verbleibenden HIGH
verteilen sich so:

\begin{center}
\begin{tabular}{rl}
\toprule
19 & Dateien ohne eigenes Gitter, warten auf die Registrierung des Zonalgitters \\
 6 & \code{depth} traegt bei zwei Dateien noch \code{units = "meter"}, bei uns \\
 5 & \code{plev7h}, unsere Modellausgabe schreibt die falschen Druckflaechen \\
 3 & Dimensionsreihenfolge bei \code{msftm} und die \code{tclm}-Zeitachse \\
 2 & \code{vegtype} und \code{basin}, beide upstream \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection*{Zwei Punkte auf unserer Seite}

Sechs Ozeanvariablen (\code{osalttend} und Geschwister) liefern wir vorerst
nicht: FESOM rechnet sie konstruktiv zweidimensional, verlangt ist
\code{(time, lev, ncells)}. Und bei \code{plev7h} schreibt unser Modell sieben
Druckflaechen, aber nicht die angeforderten sieben; \code{60000} und \code{5000}
Pa fehlen, dafuer kommen \code{100000} und \code{10000} mit. Beides ist
Modellausgabe, nicht Cmorisierung, und geht in die naechste Modellversion.
...
Der Lauf mit allem aus diesem Dokument ist durch, die Zahlen oben stammen
daraus.
