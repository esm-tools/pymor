#!/bin/bash
#SBATCH --job-name=pycmor-bench-ua-6hr-v2
...
#SBATCH --output=pycmor_bench_hr_ua_6hr_v2_%j.log
#SBATCH --error=pycmor_bench_hr_ua_6hr_v2_%j.log
...
OUTROOT=${OUTROOT:-/scratch/a/a270092/pycmor_bench_ua_6hr_v2}
...
python3 - <<PY
import yaml
y = yaml.safe_load(open("examples/cmip7_bench_hr_ua_6hr_v2.yaml"))
y["inherit"]["output_directory"] = "${OUTDIR}"
yaml.safe_dump(y, open("$PYCMOR_SCRATCH/bench.yaml", "w"), sort_keys=False)
PY

echo "=== ua_6hr_pl7h v2 (rechunked) benchmark ==="
