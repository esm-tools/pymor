#!/bin/bash
#SBATCH --job-name=pycmor-seaice-test
...
#SBATCH --output=pycmor_seaice_test_%j.log
#SBATCH --error=pycmor_seaice_test_%j.log
...
sed -e 's/dask_cluster: "slurm"/dask_cluster: "local"/' \
    ...
    examples/cmip7_core_seaice_core2_test.yaml > $PYCMOR_SCRATCH/seaice_test_local.yaml

pycmor process $PYCMOR_SCRATCH/seaice_test_local.yaml
