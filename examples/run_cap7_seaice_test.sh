#!/bin/bash
#SBATCH --job-name=pycmor-cap7-si-test
...
#SBATCH --output=pycmor_cap7_seaice_test_%j.log
#SBATCH --error=pycmor_cap7_seaice_test_%j.log
...
sed -e 's/dask_cluster: "slurm"/dask_cluster: "local"/' \
    ...
    examples/cmip7_cap7_seaice_core2_test.yaml > $PYCMOR_SCRATCH/cap7_seaice_test_local.yaml

pycmor process $PYCMOR_SCRATCH/cap7_seaice_test_local.yaml
