#!/bin/bash
#SBATCH --job-name=pycmor-veg-land-tco95-test
...
#SBATCH --output=pycmor_veg_land_tco95_test_%j.log
#SBATCH --error=pycmor_veg_land_tco95_test_%j.log

# Run pycmor process entirely on compute node (including Prefect server)
# ~55 rules: IFS/HTESSEL + LPJ-GUESS variables
...
sed -e 's/dask_cluster: "slurm"/dask_cluster: "local"/' \
    ...
    examples/cmip7_veg_land_tco95_test.yaml > $PYCMOR_SCRATCH/veg_land_tco95_test_local.yaml

pycmor process $PYCMOR_SCRATCH/veg_land_tco95_test_local.yaml
