#!/bin/bash
#SBATCH --job-name=pycmor-core-land-tco95-test
...
#SBATCH --output=pycmor_core_land_tco95_test_%j.log
#SBATCH --error=pycmor_core_land_tco95_test_%j.log
...
# Limit Dask to 4 workers (64 GB each) so data fits in memory
sed -e 's/dask_cluster: "slurm"/dask_cluster: "local"/' \
    -e '/dask_cluster:/a\  dask_n_workers: 4' \
    examples/cmip7_core_land_tco95_test.yaml > $PYCMOR_SCRATCH/core_land_tco95_test_local.yaml

pycmor process $PYCMOR_SCRATCH/core_land_tco95_test_local.yaml
