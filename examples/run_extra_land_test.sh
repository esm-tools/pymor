#!/bin/bash
#SBATCH --job-name=pycmor-extra-land-test
...
#SBATCH --output=pycmor_extra_land_test_%j.log
#SBATCH --error=pycmor_extra_land_test_%j.log
...
sed -e 's/dask_cluster: "slurm"/dask_cluster: "local"/' \
    ...
    examples/cmip7_extra_land_tco95_test.yaml > $PYCMOR_SCRATCH/extra_land_test_local.yaml

pycmor process $PYCMOR_SCRATCH/extra_land_test_local.yaml
