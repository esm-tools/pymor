...
Beide sind abgearbeitet und zusammen verifiziert. Der aktuelle Stand ist
\code{cli111}, 540 Dateien:

\begin{center}
\begin{tabular}{lrr}
\toprule
& \textbf{cli108} & \textbf{cli111} \\
\midrule
HIGH   & 1   & 1 \\
MEDIUM & 758 & 233 \\
TIME001 / TIME003 &  & 0 \\
Dateien mit falschem \code{grid\_label} &  & 0 \\
Dateien, die netCDF-C nicht \"offnen kann &  & 0 \\
\bottomrule
\end{tabular}
\end{center}
...
Der Tier \code{extra\_atm} hat jetzt 6\,h. \code{cli111}: 8760 Zeitschritte,
...
\ok\ Wie es bei uns l\"auft: nach dem Schreiben jeder Datei ruft pycmor
\code{cmip7repack -o} in-place darauf auf, und erst danach l\"auft der
Compliance-Check auf der repackten Datei. Das ist keine Sonderbehandlung f\"ur
einzelne Variablen, sondern steht \"uber \code{qc\_repack} in jedem Tier; in
\code{cli111} sind das 542 Aufrufe, einer pro Datei.

Alle drei gemeldeten Punkte sind damit weg:

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Befund} & \textbf{gemeldet} & \textbf{\code{cli111}} \\
\midrule
\code{time} & 18 Chunks & 1 \\
\code{time\_bnds} & 8760 Chunks & 1 \\
consolidated metadata & fehlte & 165725 B in einem Block \\
\bottomrule
\end{tabular}
\end{center}

Gegengepr\"uft auf genau der Datei, die du zitiert hattest:
\code{cmip7repack -x} meldet \code{"not rechunking"} f\"ur \code{time},
\code{time\_bnds} und \code{rsus}, es bleibt also nichts zu tun. Der
\code{FILE004a}-Befund zur consolidated internal metadata taucht in keinem
einzigen der \code{cli111}-QC-Reports mehr auf.
...
\code{cli111}, \code{sfcWind}: \code{coordinates = "time lat lon height"}.
...
\ok\ In \code{cli111} null Treffer, ebenso TIME003. Der Rest kam aus den
...
Der Grund, warum das mehr als eine Konvention ist: bevor wir es korrigiert hatten,
lagen f\"unf Dateien vor, die netCDF-C \emph{gar nicht} \"offnen konnte.
...
Eine Fu\ss angel dabei, die f\"ur andere Gruppen mit \"ahnlichem Aufbau vielleicht
n\"utzlich ist: wir setzen \code{grid\_label} pro Tier in einem
\code{inherit}-Block und \"uberschreiben es auf einzelnen Regeln. Diese
\"Uberschreibungen wurden still wieder auf den Tier-Wert zur\"uckgesetzt, weil die
Vererbung nach dem Zusammenbauen der Regeln noch einmal \"uber alle lief und dabei
trotz \code{force=False} zuwies. Sichtbar war das nur als Warnungen
\code{"Attribute grid\_label already exists"} im Log. Und weil zwei Regeln, die
sich nur im Gitter unterscheiden, damit denselben DRS-Pfad bekommen, hat
\code{areacella} seinen Zwilling \"uberschrieben, also eine Datei weniger als
erwartet.
...
Konzentration, die das Produkt zweier Tagesmittel verliert. Gemessen:
...
Die \S6.1-Befunde auf den Becken-Achsen haben wir aufgekl\"art. Der Checker
...
In \code{cli111} ist davon noch einer \"ubrig, der zum
