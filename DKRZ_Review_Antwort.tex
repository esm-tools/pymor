...
\code{cli112}, 540 Dateien:

\begin{center}
\begin{tabular}{lrr}
\toprule
& \textbf{cli108} & \textbf{cli112} \\
\midrule
HIGH   & 1   & 1 \\
MEDIUM & 758 & 237 \\
TIME001 / TIME003 &  & 0 \\
...
Der Tier \code{extra\_atm} hat jetzt 6\,h. \code{cli112}: 8760 Zeitschritte,
...
\code{cli112} sind das 542 Aufrufe, einer pro Datei.
...
\begin{tabular}{lll}
\toprule
\textbf{Befund} & \textbf{gemeldet} & \textbf{\code{cli112}} \\
\midrule
...
einzigen der \code{cli112}-QC-Reports mehr auf.
...
\code{cli112}, \code{sfcWind}: \code{coordinates = "time lat lon height"}.
...
\ok\ In \code{cli112} null Treffer, ebenso TIME003. Der Rest kam aus den
...
Alle vier Punkte sind umgesetzt und in \code{cli112} verifiziert.
...
In \code{cli112} stimmt bei allen 540 Dateien das \code{grid\_label} mit der
...
\section{Verbleibende Befunde in cli112}
...
115 & \code{ATTR004} \code{long\_name} & Registry weicht vom Data Request ab, s.\,u. \\
37  & \code{ATTR001} \code{cell\_measures} & Data Request gibt hier \code{''} vor \\
31  & CF \S7.2            & \code{cell\_measures} mit zwei Paaren, \\
    &                     & \code{compliance-checker\#1323} \\
26  & CF \S2.4            & Dimensionsreihenfolge \\
10  & \code{ATTR004} \code{cell\_methods} & Registry weicht vom Data Request ab \\
9   & CF \S7.1            & \code{lev\_bnds:formula\_terms}, teilen publizierte \\
    &                     & CMOR-Dateien mit uns \\
8   & \code{ATTR004} \code{cell\_measures} & Registry erwartet ihren eigenen \\
    &                     & Platzhalter \code{--MODEL} \\
...
\midrule
\code{long\_name}    & 115 & 1 & 110 (+4 ohne DReq-Eintrag) \\
\code{cell\_measures} &   8 & 0 & 8 \\
...
Bei \code{cell\_measures} lag der Fehler dagegen bei uns: der Data Request
verlangt bei 3D-Ozeanvariablen \code{"area: areacello volume: volcello"}, wir
hatten auf \code{"area: areacello"} gek\"urzt, weil der cf-checker in \S7.2 nur ein
einzelnes Paar akzeptiert, w\"ahrend CF 1.11 \S7.2 selbst mehrere erlaubt.

Wir schreiben jetzt den DReq-Wert unver\"andert, weil das die Variante ist, die am
Ende \emph{beide} Checks besteht: \code{ATTR004} verlangt sie schon heute, und
\code{ioos/compliance-checker\#1323} macht \S7.2 mehrfachpaar-f\"ahig. Solange
der PR offen ist, erscheinen daf\"ur 31 \S7.2-Befunde; sie verschwinden mit dem
Merge. K\"urzen w\"urde \code{ATTR004} dagegen dauerhaft verfehlen, denn der Data
Request verlangt beide Paare.

Die verbliebenen 8 \code{cell\_measures}-Befunde sind ein anderer Fall: dort
erwartet die Registry ihren eigenen, nicht ausgef\"ullten Platzhalter
\code{--MODEL} (\code{siu}, \code{siv}, \code{sidmasstranx/y},
\code{sistrxdtop/ydtop}), w\"ahrend wir den realm-\"ublichen Wert
\code{"area: areacello"} einsetzen.
...
In \code{cli112} ist davon noch einer \"ubrig, der zum
